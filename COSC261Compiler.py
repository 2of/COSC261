...
"""
    Hello! There is a slight bug with repeated and nested 'OR' Statements
    Due to the nature of COSC261, The risk/reward of fixing these was unjust, and
    the bugs remain!!
...
class BoolExpression_AST:
    # OR operator. if ANY term is correct goto label
...
class BoolTerm_AST:
    # AND. On any false code goto end label
...
class BoolFactor_AST:
    def __init__(self,comparison):
        self.comparison = comparison
    #    print('instatntiate boolfact')
    
    def code(self):
        ### swap the code value around
        pass
    def __repr__(self):
...
class Negate_AST:
    #negates the comparison below!
    def __init__ (self, comparison):
       
        self.comparison = comparison
    def __repr__(self):
        return 'not' + repr(self.comparison)
    #swap false and trye codes to include nots
    def false_code(self,label): 
        return self.comparison.true_code(label)
    def true_code(self,label):
        return self.comparison.false_code(label)
...
class If_Else_AST:
    ...
    def __repr__(self):
        return 'If-Else ' + repr(self.condition) + ' then ' + repr(self.then) + repr(self._else) + ' end'
...
class Or_AST:
    def __init__(self,left,right):
        self.left = left
        self.right = right
      # print(self.left, self.right,self.right.false_code('l1'))
    ...
    def false_code(self,label):
        breakcode = label_generator.next()
        return  self.left.true_code(breakcode) + \
                self.right.true_code(breakcode) + \
                'goto ' + label + '\n' + \
                breakcode + ':\n'
...
#
#   For the case of OR have a goto at the bottom that we skip over if thsi si true in anyeay
#
...
class And_AST:
    def __init__(self,left,right):
        self.left = left
        self.right = right
      #print(f'AND with {self.left} and {self.right} ')
    ...
    def false_code(self,label):
        return self.left.false_code(label) + \
              ...
              #  " --------AND ^false------ \n" + repr(self.right) + repr(self.left) +'\n'
...
class Comparison_AST:
    def __init__(self, left, op, right,flip = 0):
        self.left = left
        self.op = op
        self.right = right
    def __repr__(self):
...
def p_read(p):
    'Read : READ ID'
    p[0] = Read_AST(Identifier_AST(p[2]))
...
def p_if_else(p):
    'If : IF BoolExp THEN Statements ELSE Statements END'
...
def p_while(p):
    'While : WHILE BoolExp DO Statements END'
...
def p_if(p):
    'If : IF BoolExp THEN Statements END'
    p[0] = If_AST(p[2], p[4])


def p_comparison(p):
    'Comparison : Expression Relation Expression'
  #  print('comparison found in p_')
...
def p_boolterm(p):
    ...
    p[0] = BoolTerm_AST(p[1])
    

def p_boolfactor(p):
    '''BoolFactor : not_comparison
                | Comparison'''
    p[0] = BoolFactor_AST(p[1])

def p_not_comparison(p):
